...
%D=load('var_energy_all.csv');
% set =="x" 1 = One Peak / 2 = Two Peak / 3 = Low Energy
% s=[NDBC.time, NDBC.WDIR];
% struct2csv(s,'var_wind_direct.csv');
% D=load('var_wind_direct.csv');

% ind_op1 = NDBC.WDIR>0 & NDBC.WDIR<30;
% ind_op2 = NDBC.WDIR>210 & NDBC.WDIR<360;
%%Onshore wind
% ind_op3 = ind_op1 + ind_op2;
% ind_op = logical(ind_op3);
% spectra_days=(buoy01.DATA(ind_op,1));

ind_op = NDBC.WDIR>240 & NDBC.WDIR<360;
spectra_days=(buoy01.DATA(ind_op,1));
...
indS2 = spectra_days_vec(jj,2)==NDBC_time_vec(:,2) & spectra_days_vec(jj,3)==NDBC_time_vec(:,3) & spectra_days_vec(jj,4)==NDBC_time_vec(:,4) & spectra_days_vec(jj,5)==NDBC_time_vec(:,5);
...
title('Wave Height from SPRK Range Cell 3 and Buoy 44065 Indexed by Onshore Winds 240° to 360°')
legend(codar.name{ii},buoy.name{indB},'AutoUpdate','off')
ylabel('Wave Height (m)')
format_axis(gca,dtime.start,dtime.end,7,1,'mm/dd',0,7,1)

timestamp(1,'/Users/Jaden Dicopoulos/Desktop/matlab/statistics_NDBC_CODAR_WindDir.m')

fig = gcf;
fig.PaperUnits = 'inches';
fig.PaperPosition = [0 0 12 6];
print('-dpng','-r150',[conf.print_path 'WaveHeightTS_WDIR_OnshoreWind_' buoy.name{indB} '_' codar.name{ii}...
...
title ('Significant Wave Height (m) Scatter Plot Indexed by Onshore Winds 240° to 360°')
box on
grid on
axis equal
axis([0 3 0 3] )
...
print('-dpng','-r150',[conf.print_path 'WaveHeightCorr_WDIR_OnshoreWind_' buoy.name{indB} '_' codar.name{ii}...
    '_' dtime.startSTR '_'  dtime.endSTR '.png'])
